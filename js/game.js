/* game.js - Hotwheels Arcade Pro (UI/UX high-level + stable game loop)
   - Highly commented and modular
*/
...
const CONFIG = {
  recordsKey: 'hotwheels_records_v2',
  settingsKey: 'hotwheels_settings_v2',
  canvasId: 'gameCanvas',
  lanes: 4
};
...
const Audio = (function(){
  ...
  let settings = load();
  function load(){ try{ return JSON.parse(localStorage.getItem(CONFIG.settingsKey)) || {music:true,sfx:true,volume:0.8}; }catch(e){ return {music:true,sfx:true,volume:0.8}; }}
...
function renderCarousel(){
  ...
  CARS.forEach((c,i)=>{
    ...
    el.innerHTML = `<div style="height:120px;display:flex;align-items:center;justify-content:center;font-size:56px">${i===0?'🚗': i===1? '🏎️' : i===2? '🚙':'🚘'}</div><div style="margin-top:10px;font-weight:800">${c.name}</div>`;
...
/* render records */
function renderRecords(){ const list = $('recordsList'); list.innerHTML=''; const arr = Records.get(); if(!arr.length) { list.innerHTML='<li>No hay registros aún</li>'; return; } arr.forEach((t,i)=>{ const li = document.createElement('li'); li.textContent = `${i+1}. ${(t/1000).toFixed(2)} s`; list.appendChild(li); }); }

/* -------------------------
   UI: countdown & start flow
   ------------------------- */
const UI = (function(){
...
const Game = (function(){
  ...
  class Racer {
    constructor(name,color,x,lane,isPlayer=false){
      this.name=name; this.color=color; this.x=x; this.lane=lane; this.y=0; this.speed=0; this.maxSpeed=1.8 + Math.random()*1.1; this.isPlayer=isPlayer; this.finished=false; this.finishTime=null; this.w=58; this.h=34;
    }
    draw(off){
      const dx = (this.x / DPR) - this.w/2;
      const dy = (H / DPR) - ((this.y - off) / DPR) - 90;
      // shadow
      ctx.fillStyle = 'rgba(0,0,0,0.22)'; ctx.fillRect(dx+6, dy + this.h - 6, this.w-12, 6);
      // body
  ...
  function spawnConfetti(x,y){
    for(let i=0;i<40;i++){ particles.push({x,y,vx:(Math.random()-0.5)*6,vy:- (Math.random()*4+1),life:Math.random()*1400+400,conf:true,color:['#ff3b3b','#ffd34d','#1fd1a6','#6c5eff'][Math.floor(Math.random()*4)]}); }
  }
  ...
  function update(ts){
    ...
    $('posInfo').textContent = `Pos: ${pIndex+1}/${racers.length}`;
    ...
    $('distInfo').textContent = `Dist: ${dist}%`; $('timeInfo').textContent = `Tiempo: ${((performance.now()-startTime)/1000).toFixed(2)}s`;
  ...
  function draw(){
    ...
    // finish line
    const finishDY = (H/DPR) - ((LEVELS[levelIdx].finish - offsetY)/DPR) - 90;
...
/* -------------------------
   Finish overlay buttons + init
   ------------------------- */
$('finishReplay').addEventListener('click', ()=> { overlays.finish.classList.add('hidden'); UI.startRaceFlow(false); });
$('finishContinue').addEventListener('click', ()=> { overlays.finish.classList.add('hidden'); /* implement level continue if desired */ });
